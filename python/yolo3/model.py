...
def yolo_correct_boxes2(box_xy, box_wh):
    '''Get corrected boxes'''
    box_yx = box_xy[..., ::-1]
    box_hw = box_wh[..., ::-1]
    #input_shape = K.cast(input_shape, K.dtype(box_yx))
...
def yolo_boxes_and_scores2(feats, anchors, num_classes, input_shape):
    '''Process Conv layer output'''
    box_xy, box_wh, box_confidence, box_class_probs = yolo_head(feats,
        anchors, num_classes, input_shape)
    boxes = yolo_correct_boxes2(box_xy, box_wh)
    boxes = K.reshape(boxes, [-1, 4])
    box_scores = box_confidence * box_class_probs
    box_scores = K.reshape(box_scores, [-1, num_classes])
    return boxes, box_scores
...
def yolo_eval2(yolo_outputs,
    ...
    box_scores = []
    for l in range(num_layers): #tiny for 2 or yolo for 3
        _boxes, _box_scores = yolo_boxes_and_scores2(yolo_outputs[l],
                                                    anchors[anchor_mask[l]],
                                                    num_classes,input_shape
                                                    )
        boxes.append(_boxes)# list(3 array): [3, None*13*13*3, 4]
    ...
    for c in range(num_classes):
        ...
        boxes_.append(class_boxes)
        scores_.append(class_box_scores)
        classes_.append(classes+label_offset)
    boxes_ = K.concatenate(boxes_, axis=0) # [N, 4] with N: number of objects
    scores_ = K.concatenate(scores_, axis=0) # [N,]
    classes_ = K.concatenate(classes_, axis=0) # [N,]
